...
- Trade-off：① 个人中心的角色列表只回答「团队内角色」，无法回答「我是否 admin 全局角色」——后者需后端在 `UserRead` 加 `role`/`permissions` 字段（§6-Q1）；② N+1 请求：每个团队一次成员列表请求，团队很多时个人中心加载会变慢（当前量级可接受，已用并行 `Promise.all` 缓解）；③ 权限判断工具在权限集合落地前持续「空转」（`usePermission().can()` 恒 false），这是有意的留白而非死代码——它的单测保证了未来接线时的行为正确性。均在 `docs/FRONTEND_API_MAPPING.md` §7 标注。
