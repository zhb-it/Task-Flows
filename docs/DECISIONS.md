...
## DECISION 055 —— 权限模块交付为「能力审计 + 唯一真实落点」：不做功能级权限控制，角色按团队展示（TASK-076）

- Problem：规格 §69（第十阶段：权限）要求完成「路由权限 / 菜单权限 / 按钮权限 / 用户角色 / 权限判断 / 403 页面」六项。其中「按权限的路由/菜单/按钮控制」依赖**当前用户的权限集合**，而后端没有权限查询端点（`§4-D4`：`API_CONTRACT` 明确授权机制「无独立端点」，`UserRead` 亦无角色/权限字段）；「用户角色」同样没有全局形态可展示。若硬做，只能凭空造一份权限表——这正是 D4 判定过的「比不做更危险」。

- Decision：TASK-076 交付为两件事。① **能力审计**：§69 六项逐一对照现状——「403 页面」（阶段 1 已有路由与页面）、「路由认证守卫」（阶段 2 `guards.ts`）、「权限判断工具」（`utils/permission.ts` + `usePermission.ts`，permission.spec 10 项单测）**均已具备**；「按权限的菜单/按钮/路由控制」**明确不做**（权限集合恒为空且不据此隐藏任何入口，规格 §35「隐藏按钮 ≠ 安全」，后端 403/404 兜底），登记在 `FRONTEND_API_MAPPING.md` §7，待后端补「我的权限集合」端点（§6-Q1）后再接线。② **唯一真实落点**：「用户角色」按团队展示——个人中心新增「我的团队与角色」区块，`GET /teams` + 逐团队 `GET /teams/{id}/members` 找当前用户的 `role`（owner/admin/member），数据完全来自真实契约；每个团队独立容错（单个成员列表失败只显示「未知」，不连累其余），区块顶部注明「后端不暴露全局角色与权限集合，角色只能按团队展示」。

- Reason：① 与其交付一个假装实现了 §69 的空壳（比如用 `can('task:update')` 包一遍按钮——集合恒空会把所有按钮藏掉，或者反过来造一份假权限表让按钮全亮），不如把「哪些已具备、哪些被什么阻塞、唯一能做的是什么」说清楚；② 团队角色是后端**真实返回**的数据（`TeamMemberRead.role`），用它回答「我在这个团队里是什么」是完全诚实的，与 TASK-070 团队页按 `role` 数据驱动按钮同源；③ 逐团队串行/并行拉成员的 N+1 是当前契约下展示角色的唯一途径（无「我的团队角色」聚合端点），团队数量级小（limit 100、个人参与数通常个位数），代价可接受，且单团队失败被隔离为「未知」而非整块报错。

## DECISION 056 —— 操作日志页：筛选降级到「当前页」、不设操作人/IP 列（TASK-077）

- Problem：规格 §32 要求日志页展示「操作人 / 操作 / 资源 / IP / 时间」五列，支持「时间筛选、操作人筛选、操作类型筛选、分页」。但 `GET /logs` 参数只有 `skip`/`limit`（`limit` 上限 100），响应裸数组**无 total、无任何筛选参数**；`OperationLogRead` 只有 `id / user_id / resource_type / resource_id / action / payload / created_at`，**没有 IP 字段**（审计埋点也未记录 IP）；且 `GET /logs` 语义是**仅当前用户自己的时间线**——「操作人筛选」与「操作人列」天然无意义。

- Decision：**按真实契约交付，超出部分显式降级**（登记 `FRONTEND_API_MAPPING.md` §4-D16）。① 列表接 `GET /logs`，分页「上一页/下一页」，以「本页取满 pageSize」推断有无下一页；② 操作类型/时间筛选做在**当前已取回的页**上（客户端过滤），工具栏与页脚注明「仅作用于当前页，翻页需重新应用」；③ 不设操作人列与 IP 列，顶部 `el-alert` 一次性写明四点降级原因；④ `payload` 按后端当前实际写入的三种 action 收窄为人类可读描述（`task:transition` → 状态 x→y、`comment:delete` → 任务 #x 中的评论 #y、`attachment:delete` → 任务 #x 的附件「名」），未知 action 原样展示、payload 走 JSON 兜底；⑤ 顺带给 `api/log.ts` 补 `listResourceLogs`（`GET /logs/{type}/{id}`，资源级日志，供后续任务详情「日志」页签等场景消费，当前页面未用）。

- Reason：① 降级点全部**可恢复**：后端补筛选参数/total/IP 字段后，前端把客户端过滤换成服务端参数、补列即可，数据结构不变；② 「按当前页过滤」是有明确语义的次优解（用户至少能对刚看到的一页做定位），比「藏起筛选假装没这需求」或「一次拉 100 条假装全量」都诚实——后者在数据超过 100 条时会静默丢数据；③ payload 收窄只认「确认过源码的三种 action + 字段存在性守卫」，未知形状一律 JSON 兜底，呼应 §44「不能假定 payload 里有固定字段」的类型注释。

- Trade-off：① 筛选只覆盖当前页，跨页筛选用不了（已在页面两处注明）；② 无 total，「下一页」按钮以「上一页是否取满」启发式启用，末页恰好满页时会多出一次空页请求（请求本身无害，返回空数组后按钮即禁用）；③ 时间筛选比较的是 ISO 字符串字典序，依赖后端统一输出带时区的 ISO 8601（当前成立）；④ 「资源」列只显示类型+id（如「任务 #123」）不可点击——资源详情路由需要项目上下文（任务路由为 `/projects/:projectId/tasks/:taskId`），日志里没有 project_id，做跳转又是一层正文猜测，与 D15 同理不做。均在 `docs/FRONTEND_API_MAPPING.md` §7 标注。

## DECISION 057 —— 前端测试只做单元测试：组件测试按真实组件形态降级，E2E 按既定口径挂起（TASK-078）

- Problem：规格 §71（第十二阶段：测试）要求三类测试——**单元测试**（重点：utils / composables / stores / permission / format）、**组件测试**（重点：Login / TaskForm / TaskCard / TaskFilter / Pagination）、**E2E**（注册 → 登录 → Dashboard 核心流程）。前五项单元测试重点在本代码库均有真实对应物；但组件测试点名的五个组件里只有 Login 存在独立文件，TaskForm / TaskCard / TaskFilter / Pagination **不是独立组件**——表单内嵌在视图（TaskCreate/TaskDetail），卡片是表格行（TaskList），筛选是工具栏（TaskList/日志页），分页是「上一页/下一页」按钮组——按组件名硬造五个壳组件来「满足清单」就是为文档而文档；E2E 则需要真实后端与真实登录，登录冒烟受本机安全策略（口令字面量）拦截是 Phase 11 起的既定口径。

- Decision：TASK-078 交付**单元测试**并把 §71 五个重点全部落地（37 → 70 项，8 个 spec）：format（空值/非法值降级、相对时间五档边界用 `vi.setSystemTime` 固定「现在」、文件大小 1024 进制与小数位收敛）、composables（`usePermission` 钉住「权限集合恒为空」——谁把假权限表塞进来测试立刻红）、stores（auth：令牌唯一事实来源是 storage、登录失败不留半截状态、撤销失败不阻塞登出；notification：预览必须 `silent`、未读数前端统计、已读本地同步翻转）、permission 与 utils（storage/request/router 守卫）此前已有。**组件测试不做**，理由如上登记在案；若未来视图重构出真正的表单/卡片组件，再按 §71 补挂载测试。**E2E 不做**，沿用 Phase 11 口径（登录冒烟被安全策略拦截不绕过），后端全量 1011 项测试已覆盖同样的业务规则。

- Reason：① 单元测试的价值在**钉契约**而非凑数量——四个新 spec 各自守着一条会真实漂移的决策（假权限表、silent 回退、令牌双份、时间格式时区假设），这些正是本项目反复踩过的坑；② 「组件测试」的清单与真实代码形态错位，说明规格 §71 写于组件库形态未定时；照单造组件是「为满足验收而造假」，与 D4/055 的纪律同源；③ 测试基建已就位（@vue/test-utils + jsdom 都在 devDependencies），补组件测试的唯一前置条件是「出现值得挂载测试的独立组件」——这是可恢复的降级。
...
## 061 用户搜索与邀请选择器（TASK-085~086）

- Date：2026-09-16
- Context：邀请成员/分配角色都要求提供 `user_id`，但 `GET /users` 只能给一份按 id 排序的裸列表——管理员无法判断某个自增 id 对应哪个用户（用户反馈）。后端有唯一约束保证 username/email 精确唯一，但「知道名字、拿不到 id」仍是断点。
- Decision：
  1. **搜索挂既有列表端点，不新增资源**：`GET /users?q=` 按用户名/邮箱子串、大小写不敏感过滤（`ILIKE '%q%'`，与任务搜索同口径）；不新建 `/users/search`，不新增权限项（与列表同挂 user:read——member 本就持有，协作找人场景一致）。
  2. **空白视同不过滤**：`q` 为空串/纯空白时行为与不带参一致，前端选择器「打开即预载前 50 个」复用同一参数。
  3. **选择器替人记 id**：前端邀请对话框改为 el-select 远程搜索，选项展示 `用户名（#id · 邮箱）`，选中值即 user_id——id 只在提交时出现，人只需要认名字。
- Consequence：邀请/角色分配全程可见「这个人是谁」；代价：`ILIKE '%q%'` 不走普通索引，用户表量级预期小（团队内部系统），量级增长后可评估 pg_trgm 表达式索引。

## 062 企业化规划的四个边界，以及「未勾选任务」护栏的演进（TASK-088 起规划，用户确认）

- Date：2026-09-16
- Context：TASK-087 之后项目进入「企业可直接使用」评估（`docs/ENTERPRISE_READINESS.md`）。实测结论是地基（分层纪律 / 测试真实性 / 部署暴露面 / 越权防护）扎实，缺口集中在可观测、可运维、身份体系、合规与产品可用性五层。项目规则 §3 禁止猜测需求，所以新增能力必须先登记规格；同时 `docs/TASKS.md` 一直不存在未勾选任务，护栏 `tests/test_readme.py` 里「未勾选任务必须为空」的断言会与新规划直接冲突。
- Decision：
  1. **四个边界（用户拍板）**：目标形态 = **多租户 SaaS**；交付底座 = **Docker Compose 与 Kubernetes 都要**；身份档位 = **本地账号加固 + MFA（TOTP）+ 企业目录（OIDC/LDAP）**；**不做「最小可交付版」**。其余四项（邮件通道、附件存储、合规档位、分页口径）按本方案默认值推进，并在对应 TASK 内标注「待决策」，不阻塞前序 Phase。
  2. **新增需求登记为规格 §61**：租户层、MFA/SSO、邮件、备份、指标、CSP/TLS、数据主体权利、`total` 分页、对象存储、K8s 清单等以 §61 各小节形式进入规格事实来源（`团队任务协作系统_项目开发文档.md`），并明确「§61 未列出的企业能力仍不在范围内」。
  3. **顺序：可靠性 → 多租户 → 身份与安全 → 合规 → 产品 → 工程交付**。理由：Phase 18 与租户无关且立刻降低上线风险，并为 Phase 19 的结构性改造提供可观测性（改造期间最需要指标与健康检查）；Phase 20 的认证与权限必须建立在租户模型之上，否则同样的改动要做两遍。
  4. **护栏演进：换一条更强的性质，而不是放宽**。原断言 `pending == []` 要防的是「README 的前沿声明说谎」；现在改为——① `scripts/check_docs.py` 新增第 6 条不变量：已勾选编号必须是 `1..max` 的**连续区间**（有空洞 = 中间有任务被跳过，而前沿声明把它盖住），且缺口号若被登记成未勾选任务则归因为「补登记历史任务」、若其下无任何条目则归因为「漏登记」；② `tests/test_readme.py` 改为：有未勾选任务时 README **不得**声称「无未完成任务」。两条反向用例（`BACKFILLED_TASKS` / `HOLE_TASKS`）保证新不变量不空转。
- Consequence：TASK-088~127（6 个 Phase、40 项）成为正式待办，`## Next` 指向 TASK-088。风险与代价必须记住：**多租户是唯一的结构性改造**——触及全部业务表的 `tenant_id`、原先的全局唯一约束、RBAC 租户化、认证令牌与全部资源型端点的测试，不是加法；其余 35 项是加法。方案里的默认值（邮件通道 = 可插 SMTP、附件存储 = 对象存储、合规 = 个保法 + GDPR 口径、分页 = 显式 `COUNT(*)`）在被推翻前即按此实现，推翻时须同步本文与对应 TASK。

## 063 生产配置自检的最小密钥长度与「收集全部再拒绝」策略（TASK-091）

- Date：2026-09-16
- Context：§51 补充要求 `APP_ENV=production` 启动自检并拒绝四类错配（DEBUG=true / 默认或过短 JWT 密钥 / 默认数据库口令 / 开信任代理但网段为空），但规格未给「长度不足」的数值，也未约束报错策略——逐条 fail-fast 会让运维「改一项撞一项」。
- Decision：
  1. **最小密钥长度取 32 字符**：`jwt_secret_key` 用于 HMAC-SHA256（HS256）签名，密钥至少应与其输出等长——32 字节 = 256 bit，与 HS256 安全强度对齐；低于它视为「长度不足」拒绝。等于默认值 `change-me` 的情形单独点名（语义不同：不是弱，是公开）。
  2. **默认口令判定用 `:postgres@` 子串**：精确匹配「口令字面为 postgres」的连接串（`postgres-2024` 这类不误伤）；用户名任意。
...
## 069 个人工作台聚合：一个端点 + 明写分母 + 「本周完成」用 updated_at 近似（TASK-129）

- Date：2026-09-17
- Context：开发文档 §61.6 登记了两条欠账——「个人视图：提供跨项目的『我的任务』与工作台聚合」
  与「统计：进度口径（**分母定义必须写明**）」。前端方案（`docs/frontend-ux-plan.md`）
  Phase C 要做 Bento 首页，一屏要六个数字；若按旧做法让前端连打四五个端点，既慢又会
  半屏半屏地闪。三个未定稿点：①统计的分母是什么（全部任务？我的任务？我可见的项目？）；
  ②`tasks` 表没有 `completed_at`，「本周完成」用什么时间；③周起点按哪个时区。
- Decision：
  1. **一个端点 `GET /users/me/overview` 一次返回首页全部数字**，只要求登录。
     理由：首页是「一屏聚合」场景，一次请求换一次加载态；前端不做二次统计，避免
     两套口径。代价是这个端点与首页卡片绑定，卡片增减要同步改契约——可接受，因为它
     换来的是「数字只有一处定义」。
  2. **分母明写并在三处同步**（Service 模块文档 / `docs/API_CONTRACT.md` / 前端页面脚注）：
     可见范围 = 用户所属团队下的项目；`task_status.*` 是**可见范围下的全部任务**（团队
     概况口径）；`my_tasks.*` 是**可见范围 ∩ 分配给我**（个人视图口径）；`overdue` 是
     `assigned_open` 的**子集**而非并列集合。响应回传 `week_start`，前端不得自行推算周起点。
  3. **「本周完成」用 `updated_at` 近似完成时刻，并显式记录为近似**。依据是状态机
     `DONE` 为终态、进入后不能再流转，故「最后一次更新」通常就是完成时刻。残留误差：
     进入 `DONE` 后再被普通 PATCH 修改字段会推后完成时刻。**不**顺手加 `completed_at`
     列（那是一次迁移 + 回填，属独立决策）；先把近似写在契约与页面脚注里，让误差可见。
  4. **周起点按 UTC**，与令牌签发、维护任务的时间口径一致；租户时区是 §61 待落地能力，
     不提前引入。
  5. **聚合不缓存**：工作台要求「刚做完的事立刻可见」，几条形如 `COUNT(*)` 的成本远低于
     陈旧感。将来若规模增长需要缓存，须作为显式决策记录。
  6. **端点只做读**：不得因为「顺手」把通知标记成已读——那会让前端未读红点自己消失。
     这一条写成了回归测试。
- Consequence：首页终于有真实数字，前端方案 D8/Q2 两条欠账关闭；代价是「本周完成」
  在极端路径下不精确，以及端点与首页卡片形状耦合。二者都已写进文档而非藏起来。

## 070 前端体验升级的任务编号：登记为 TASK-128~130 但**暂不勾选**（TASK-128/129 已交付）

- Date：2026-09-17
- Context：用户把优先级从企业化主线（TASK-097~127）切换到前端体验升级，Phase A/B/C
  已交付并验证。按 `docs/TASKS.md` 的既有做法，新需求追加到序列尾部即 TASK-128 起。
  但 `scripts/check_docs.py` 的不变量 #6 要求已勾选任务编号是 `1..前沿` 的连续区间——
  勾掉 128/129 会在 97~127 处造出空洞，等同于对外宣称「已交付到 TASK-129」。
- Decision：
  1. **照常登记为 TASK-128~130（Phase 24），保持未勾选**；交付说明写在
     `docs/PROGRESS.md` 的同名小节与 Project Status 里，代码、测试、文档都真实落地。
  2. **不顺手重排序列**：把 097~127 整体下移会让 `docs/ENTERPRISE_READINESS.md` 的
     Phase↔编号表、DECISIONS 里的多处引用、README 的 Phase 声明一起漂移，属独立决策。
  3. 勾选待前沿推进到此处，或待序列被正式重排。
- Consequence：避免了「宣称交付到 TASK-129」的虚假前沿；代价是已完成的工作在
  `docs/TASKS.md` 里仍显示为未完成，读者必须读 PROGRESS 才知道已交付——这一点已在
  TASKS.md 的 Phase 24 说明里点名，不留暗坑。

## 071 给测试加挂起护栏：`faulthandler_timeout = 180`（TASK-129 收尾）

- Date：2026-09-17
- Context：一次带覆盖率测量（`pytest --cov`）的全量运行**卡在 87% 上 7 小时**不结束，
  输出停在最后一个完成的点上、没有任何「正在跑哪个用例」的信息；同上一次（TASK-096
  期间的脏库运行）一样，事后单独重跑全部相关文件都是绿的，**不可复现**。同时暴露的是
  一个更普遍的问题：pytest 默认对挂起零作为，而挂起恰恰是**最没有可观测性**的失败
  形态——它不报错、不退出，只是把时间烧掉（CI 上要烧到 job 的 6 小时上限）。
- Decision：在 `pyproject.toml` 的 `[tool.pytest.ini_options]` 里设
  `faulthandler_timeout = 180`（pytest 内置 faulthandler 插件，无新依赖）。
  1. **阈值 180s 而非「按用例调的短超时」**：全量基线约 290s（带覆盖率）/ 550s
     （不带），最慢的用例在秒级；180s 是「绝不可能是正常耗时」的量级，因此**不会误杀**，
     也就不需要为个别慢用例留豁免清单——豁免清单本身会腐化。
  2. **触发即失败**：dump 全线程栈并以非 0 退出码结束，而不是只打印警告继续跑。
     挂起必须让运行显式失败，否则「静默烧时间」这个失败模式原样保留。
  3. **不引入 `pytest-timeout`**：它需要新依赖（本项目依赖是钉版本的），而这里要的是
     「卡住时给我栈」，faulthandler 正好就是这个语义；`--timeout` 的「中断再继续」还
     会把挂起伪装成一批失败，反而更难定位。
  4. **护栏不替代定位**：两次挂起都归因于**环境性的连接/会话残留**（被中止的运行把
     连接池与会话留在中间态），重启运行即恢复；因此没有为它编写回归用例——一个既不能
     稳定复现、又依赖外部进程状态的现象，写了也只是假绿。
- Consequence：以后挂起会**当场报出卡住位置**并让运行失败，不再有「跑几小时不知道在
  干什么」的形态；代价是极端慢的机器上若真的存在 >180s 的单用例，会被误判为挂起——
  届时应当先确认它是不是真慢，而不是直接调大阈值。

---

## 072 前端体验收口：未定义令牌护栏、断点抽 composable、空态分流与真实计数引导（TASK-130）

- 背景：TASK-130（方案 Phase D）实施前的走查中，发现上一轮 Phase A/B 留下**四类问题**，
  其中前三类是「四门全绿也照样存在」的缺陷：

  1. **引用了一整套从未定义的 CSS 变量**：`CommandPalette.vue` 与 `AppHeader.vue` 用了
     `--tf-border` / `--tf-bg` / `--tf-surface` / `--tf-text` / `--tf-text-muted` /
     `--tf-radius-lg` / `--tf-shadow-lg` / `--tf-overlay`，而 `tokens.css` 里只定义了
     `--tf-header-height` / `--tf-sidebar-width` / `--tf-sidebar-collapsed-width`。
     CSS 变量未定义**不报错**，整条声明在计算值阶段失效：命令面板成了透明底、方角、
     无阴影的浮层，`color-mix(..., var(--tf-overlay) ...)` 连遮罩一起失效。
     typecheck / lint / build / 单测四项**没有任何一项能发现它**。
  2. **暗色下的硬伤**：`NotificationBell.vue` 写死了 `#303133`（文字）/`#e4e7ed`（分隔线）
     ——明色的深灰字落在暗色面板上几乎不可见。同类魔法色还散落在 `AuthLayout`、
     `Login` / `Register`、`PermissionManage`、`Dashboard`。
  3. **空态把两件事混成一句**：`ProjectList` 的「还没有项目，或筛选无匹配」让用户无法
     判断是数据没了还是自己的筛选太严，也就不知道下一步点「创建项目」还是「清除筛选」；
     其余列表页是裸的 `empty-text="暂无日志"` 之类，没有任何出口。
  4. **窄屏只有折叠、没有抽屉**：规格 §73 的「<992px 自动折叠」在平板/手机宽度下站不住
     ——64px 固定占位 + 内容区最小宽度会挤出横向滚动条，图标条对不熟悉产品的人等于没有导航。

- 决定：

  1. **把「引用了未定义的令牌」做成单测护栏**（`tests/unit/design-tokens.spec.ts`，三条
     不变量）：① 源码里 `var(--x)` 的每个 `--x` 都必须在自己的 CSS / `<style>` 里定义过
     （`--el-*` 豁免，那是 Element Plus 运行时提供的）；② 主题相关令牌必须在 `:root` 与
     `html.dark` **两块都定义**（清单显式列举，不用正则猜）；③ 组件/页面样式里不出现
     十六进制或 `rgb()/rgba()/hsl()` 颜色（`tokens.css` 是令牌来源，豁免）。
     理由：这类缺陷**只能靠眼睛发现**，而「靠眼睛」在有 60+ 个 `.vue` 的项目里等于不存在；
     护栏把「令牌是唯一事实来源」这句承诺变成了可执行的断言。
  2. **顺手把魔法颜色清零**，并新增 `--overlay-backdrop`（遮罩）与 `--bg-auth`（认证页
     渐变，暗色换深底，避免用 `brand-50` 导致整页发亮）两个真的需要主题差异的令牌。
  3. **断点抽成 `composables/useBreakpoint.ts`**（`MOBILE_BREAKPOINT = 992`、
     `DRAWER_WIDTH = '240px'`），布局组件只保留「布尔 → 抽屉 or 常驻侧栏」的渲染分支。
     断点逻辑因此可以**不挂载组件**直接测（`effectScope()` 里跑，用 `resize` 事件驱
     991 / 992 / 1440 三个点），既覆盖了「跨断点跟随变化」，也覆盖了「作用域销毁后注销
     监听」（避免热更新/重复挂载后的重复监听）。窄屏是 **DOM 结构切换**（`el-aside` ↔
     `el-drawer`），不是靠 CSS 折叠——这是 TASK-130 与旧实现的实质区别。
  4. **空态一分为二**：新增 `components/common/EmptyState.vue`（内联 SVG 插画 + 标题 +
     描述 + `#actions` 出口），各列表页按「无数据 / 被筛掉」分流，前者给创建动作，后者给
     「清除筛选」；表格类页面走 `#empty` 槽。同时**删除死代码 `PagePlaceholder.vue`**
     （0 引用，`components.d.ts` 之外无人提及；留着它只会让令牌护栏要么豁免、要么养一份
     旧样式），并同步 `frontend/README.md` 的目录说明。
  5. **引导用真实计数**：`GET /users/me/overview` **增加 `teams` 字段**（我加入的团队数），
     新账号引导据此判断「建团队 → 建项目 → 分任务」三步各自是否完成，只强调第一个未完成
     的步骤。这是**加法**（不改既有字段语义、不动数据模型），选它而不是「前端再打一次
     `GET /teams`」，是因为该端点的设计意图就是「一次请求取齐首页所需」。
     顺带在 `tests/test_me_overview.py` 里把 `teams ≠ projects` 钉死（第二个团队没有项目
     时 `teams = 2, projects = 1`），避免后来者把两者当成同一个数。

- 视觉实证（2026-09-18 补记）：本轮的教训是「四门全绿 ≠ UI 是对的」，所以交付前用无头
  Chrome 走了一遍**真实渲染**，而不是再读一遍代码。做法记在这里，因为踩过坑：
  用 CDP 的 `Emulation.setDeviceMetricsOverride` 设视口，**不要用 `--window-size`**——
  Windows 上窗口有约 500px 的最小宽度，传 `390` 实际是按 500px 布局、再裁成 390px 输出，
  会让人误判「移动端被切掉了」（其实布局是好的）；判据是量
  `documentElement.scrollWidth` 与 `innerWidth` 是否相等，390 / 1440 两档实测相等。
  实证又抓出两处：
  1. `Register.vue` 的「确认密码」漏了 `required` 规则。**校验逻辑没错**（validator 自己拦
     空值），但 Element Plus **只在规则里存在 `required` 时才渲染必填星号**——同一张表单上
     四个必填字段里唯独它没有星号，用户会以为可以不填。已把 `required` 与 validator 并列。
     这类「逻辑对、呈现错」的缺陷，只有把页面渲染出来才看得见。
  2. `BasicLayout.vue` 的注释声称有 `tests/unit/responsive.spec.ts` 守着「窄屏渲染抽屉 /
     宽屏渲染常驻侧栏」这条契约，而该文件**根本不存在**——护栏只「写在注释里」。已把它补
     成真文件（10 项断言：两种形态互斥、跨断点整体互换、抽屉宽度取自 `DRAWER_WIDTH` 常量、
     窄屏开合抽屉 / 宽屏折叠侧栏、窗口变宽时抽屉自行关闭、顶栏拿到的 `isMobile` 与形态一致）。
     **由此形成第二条纪律：注释里不得声称存在某个测试文件，除非它真的在仓库里。**
- Consequence：
  - 「写了未定义变量、样式静默失效」与「暗色下漏改颜色」这两类回归从此**测挂**，不再依赖
    走查；代价是新增令牌要同时满足三条不变量（多写几行，但写错会立刻知道）。
  - 空态从「一句说明」变成「说明 + 出口」，每个空页面都有可执行的下一步；代价是各页多出
    分别维护的文案——文案是产品资产，放在页面里而不是组件里，是有意为之。
  - 首页多了一个字段的查询；引导的完成状态来自后端真实计数，所以「我建过团队了」不会因为
    换浏览器或清缓存又被要求做一遍。
  - **仍未做（本 Decision 不覆盖）**：D1 的分步 tour（本轮只做「首屏三步卡 + ⌘K 提示」，
    没有引入 `intro.js` 之类的引导库）、D2 微动效体系（只保留了既有的路由淡入与卡片悬浮）、
    D3 的对比度机器校验（暗色打磨靠令牌化 + 人工走查）、以及从未落地的 E2E（DECISIONS 057
    的挂起口径不变）。
