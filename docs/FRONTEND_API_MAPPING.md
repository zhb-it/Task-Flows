...
- **现状（TASK-129 后）**：新增 `GET /api/v1/users/me/overview`，一次返回首页所需的
  全部聚合数字（团队数 / 项目数 / 任务状态分布 / 我的待办 / 逾期 / 本周完成 / 未读通知 /
  最近项目）。口径（分母定义）写在 `docs/API_CONTRACT.md` 与
  `app/services/overview.py` 的模块文档里。
- **TASK-130 补充**：其中 `teams`（我加入的团队数）是为「新账号引导」加的——引导要判断
  「第一步建团队」是否完成，否则前端得为此再打一次 `GET /teams`，与该端点「一次取齐」
  的设计意图相悖。注意 `teams` 与 `projects` **不是同一个数**（建了团队还没建项目时是
  `1 / 0`），后端用例已把这一点钉死。
...
| 操作日志 | 阶段 13 | D16（无筛选参数 / total / IP） | **已实现（TASK-077）**：`OperationLogList.vue` 接 `GET /logs`（当前用户时间线，skip/limit，limit 上限 100）；操作类型 / 时间筛选做在当前已取回的页（后端无筛选参数）、页面明示；分页仅「上一页/下一页」；操作人列（时间线即本人）与 IP 列（schema 无 IP 字段）不设、页面明示；payload 按 action 收窄（三种已知 action，未知 JSON 兜底）；`api/log.ts` 补 `listResourceLogs`（资源级日志 `GET /logs/{type}/{id}` 备用，当前页面未消费） |
