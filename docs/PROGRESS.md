...
## Project Status
Completed——`docs/TASKS.md` 中 TASK-001 ~ TASK-075 全部勾选，无未完成任务（后端 TASK-001~064 + 前端 TASK-065~075）。

## Current Phase
Phase 12：前端业务页面

## Current Task
TASK-075 通知模块（规格 §30/§31「通知」，阶段 11：NotificationList.vue 占位替换为真实收件箱——GET /notifications（skip/limit 最新在前）+「全部/未读/已读」客户端三页签（后端无 is_read 筛选参数，D9）、单条 PATCH /notifications/{id}/read 与全部 PATCH /notifications/read-all 经 stores/notification.ts 调用以同步顶栏铃铛、分页仅「上一页/下一页」（响应无 total）；「点击通知跳转对应资源」诚实降级——NotificationRead 无 link/resource_id 字段、资源 id 仅以文本嵌在 content，前端不解析正文猜 id（D15、DECISIONS 054）；types/notification.ts 补类型中文标签（后端实际写小写 task_assigned/task_status_changed，与规格 §30 大写不一致，以真实契约为准）；顺带修正 stores/notification.ts 的 silent 注释与代码不一致（listNotifications 增 options 参数并传 silent:true）
...
无——TASK-001 ~ TASK-075 全部交付（后端 TASK-001~064、前端 TASK-065~075），`docs/TASKS.md` 中已无未勾选任务。前端后续业务页面（规格 §59 的阶段 12~16：权限 / 日志 / 测试 / Nginx / 优化）将在后续轮次按 §59 顺序登记 TASK-076 起；其中阶段 12 权限页受 `docs/FRONTEND_API_MAPPING.md` §4-D4「无权限集合端点」阻塞，需后端补端点或按现状降级，阶段 13 操作日志页接 `GET /logs` 即可、无已知阻塞项。
## 部署状态
