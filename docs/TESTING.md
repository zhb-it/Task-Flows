...
## 测试挂起护栏（faulthandler_timeout，TASK-129 收尾）

`pyproject.toml` 的 `[tool.pytest.ini_options]` 里设了 `faulthandler_timeout = 180`：任何单个用例超过 180s，pytest 会 dump **全部线程的栈**并以非 0 退出码结束本次运行。

- **为什么需要**：pytest 默认对「卡住」毫无作为，而且 `-q` 的输出停在**最后一个完成的点**上，看不出正在跑什么。没有护栏时，本地会表现为无人值守地跑几小时（实际发生过一次，7 小时后仍停在 87%），CI 上则是耗到 job 的 6 小时上限才被杀，两种情况下**都拿不到定位信息**。有了护栏，stderr 里直接给出卡住位置的完整调用栈。
- **为什么是 180s**：全量基线约 290s（带覆盖率）/ 550s（不带），最慢的用例（Celery eager 任务体、迁移可逆性）在秒级。180s 是「绝不可能是正常耗时」的量级，不会误杀，也不需要为个别慢用例加豁免。
- **它不替代定位**：护栏只负责「别静默挂着」。真触发时应当去看那段栈找根因（本项目那次的成因是环境性的——被中止的运行留下了连接/会话残留，重启运行即恢复；重现方式与结论记在 `DECISIONS.md` 071）。
...
- `design-tokens.spec.ts`——三条不变量：① 源码里 `var(--x)` 的每个变量都必须在自己的
  ...
  ② 主题相关令牌（面/边框/文字/阴影/遮罩/认证底色，清单显式列举）必须在 `:root` 与
  `html.dark` **两块都定义**；③ 组件与页面样式里不出现十六进制颜色或 `rgb()/rgba()/hsl()`
  （`tokens.css` 是令牌来源，豁免）。
  **为什么需要**：CSS 变量未定义**不报错**，整条声明在计算值阶段静默失效——`typecheck` /
  `lint` / `build` / 既有单测四项全都能放过它。这不是假想：命令面板曾引用一整套从未定义的
  `--tf-*` 变量，于是成了透明底、方角、无阴影的浮层，遮罩也一起失效，而四门全绿。
- `use-breakpoint.spec.ts`——断点逻辑是纯函数级的：`effectScope()` 里跑，改
  `window.innerWidth` 再派发 `resize`，覆盖 991 / 992 / 1440 三点（断点内侧、断点本身、
  宽屏）与「作用域销毁后注销监听」（防热更新/重复挂载后重复监听）。
