"""Auth schemas — login / refresh / logout requests and token responses.
...
双 Token / §55.1 登录流程). `RefreshRequest` and `LogoutRequest` carry the
Refresh Token in the body — the project has no cookie/session mechanism, every
credential travels through an explicit request field.
"""
...
class LogoutRequest(BaseModel):
    """Body of `POST /api/v1/auth/logout` (TASK-019)."""
